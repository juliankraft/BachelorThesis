...
This leaves four categories for the classification task, which are summarized in \autoref{tab:class_labels}.
    ...
    Since the model outputs a normalized confidence value\todo{Sind es nicht eher log-probabilities? Am Ausgang nutzt du ja wahrscheinlich Cross-entropy oderso} over classes, the values in each row sum to one, i.e., \( \sum_{j=1}^{4} p_{i,j} = 1 \) for all \( i \in \{1, \ldots, n\} \).
    ...
    The p-value was not reported, as it becomes uninformative with large sample sizes, where even negligible correlations tend to appear statistically significant.\todo{back up with a source}
